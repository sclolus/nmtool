...
{
	if (!ft_memcmp(ofile->archive_member_header_addr, LONG_ARCHIVE_NAME_MAGIC,
									sizeof(LONG_ARCHIVE_NAME_MAGIC) - 1))
	{
		...
		ofile->archive_member_header.member_name = (uint8_t*)
			ofile->archive_member_header_addr + 60;
...
int32_t					archive_parse_member_header(t_ofile *ofile)
{
	...
	ofile->archive_member_header.st_uid = ft_atou((const char *)member_hdr_addr
												+ 28);
	ofile->archive_member_header.st_gid = ft_atou((const char *)member_hdr_addr
												+ 34);
	ofile->archive_member_header.st_mode = ft_atohu_base(
		(const char *)member_hdr_addr + 40, "01234567");
	ofile->archive_member_header.st_size = ft_atoll(
		(const char *)member_hdr_addr + 48);
